  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="驱动编号" prop="driverNum" width="120" />
    ...
    {{ filterDict(scope.row.driverType,driver_typeOptions) }}
    </template>
</el-table-column>
    ...
    {{ filterDict(scope.row.status,statusOptions) }}
            ...
            <el-table-column align="left" label="创建时间" prop="createdTime" width="180">
   ...
   <template #default="scope">{{ formatDate(scope.row.updateTime) }}</template>
            ...
            <el-table-column align="left" label="设备类型" prop="deviceCategory" width="120" />

            <el-table-column align="left" label="驱动编号" prop="driverId" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updateWlDriversFunc(scope.row)">编辑</el-button>
    ...
    <el-drawer destroy-on-close :size="appStore.drawerSize" v-model="detailShow" :show-close="true" :before-close="closeDetailShow" title="查看">
            <el-descriptions :column="1" border>
                    <el-descriptions-item label="驱动编号">
    {{ detailFrom.driverNum }}
...
const getTableData = async() => {
  const table = await getWlDriversList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteWlDriversByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateWlDriversFunc = async(row) => {
    const res = await findWlDrivers({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteWlDriversFunc = async (row) => {
